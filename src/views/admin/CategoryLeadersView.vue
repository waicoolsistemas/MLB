...
async function fetchAllStats() {
  ...
  try {
    ...
    const results = await Promise.all(
      teams.map(team =>
        leagueService.getPlayerStatsByTeam(categoryId.value, team.id)
    ...
    error.value = e.response?.data?.error || 'Error al cargar lideres'
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="card p-6">
    <div class="flex items-center justify-between mb-6">
      <div>
        <h2 class="text-lg font-semibold text-white">Lideres de Categoria</h2>
        <p v-if="allStats.length > 0" class="text-gray-500 text-xs mt-0.5">
          {{ allStats.length }} jugador{{ allStats.length === 1 ? '' : 'es' }} con estadisticas
        </p>
      </div>
      <button
        @click="fetchAllStats"
    ...
    <div v-else-if="allStats.length === 0" class="text-center py-12">
      ...
      <p class="text-gray-500 text-sm max-w-sm mx-auto">
        Aun no hay estadisticas registradas en esta categoria. Los lideres apareceran cuando se capturen metricas de juegos.
      </p>
    </div>
