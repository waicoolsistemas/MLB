...
onMounted(async () => {
  error.value = null
  try {
    await store.fetchPlayoffStandingsIfNeeded(categoryId.value)
  } catch (err: unknown) {
...
async function handleRefresh() {
  error.value = null
  try {
    await store.refreshPlayoffStandings(categoryId.value)
  } catch (err: unknown) {
    const e = err as { response?: { data?: { error?: string } } }
    error.value = e.response?.data?.error || 'Error al cargar posiciones'
  }
...
<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <div>
        <h2 class="text-lg font-semibold text-white">Posiciones de Playoffs</h2>
        <p v-if="store.playoffStandings.length > 0" class="text-gray-500 text-xs mt-0.5">
          {{ store.playoffStandings.length }} equipo{{ store.playoffStandings.length === 1 ? '' : 's' }}
        </p>
      </div>
      <button
        @click="handleRefresh"
        :disabled="store.playoffStandingsLoading"
        class="btn-secondary flex items-center gap-2 text-sm"
      >
        <svg
          class="w-4 h-4 transition-transform"
          :class="{ 'animate-spin': store.playoffStandingsLoading }"
    ...
    <div v-if="store.playoffStandingsLoading && store.playoffStandings.length === 0" class="space-y-2">
      <div class="bg-navy-900 border border-navy-700 rounded-lg p-4 animate-pulse" v-for="i in 5" :key="i">
        <div class="flex items-center gap-4">
          ...
          <div class="flex-1"><div class="h-4 bg-navy-700 rounded w-32"></div></div>
          <div class="flex gap-6"><div class="h-4 bg-navy-700 rounded w-6" v-for="j in 7" :key="j"></div></div>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="store.playoffStandings.length === 0" class="text-center py-12">
      ...
      <p class="text-gray-500 text-sm max-w-sm mx-auto">
        Registra resultados de juegos de playoffs para ver la tabla de posiciones.
      </p>
    </div>
    ...
    <div v-else class="overflow-x-auto">
      <table class="w-full text-sm">
        ...
        <tbody>
          <tr
            v-for="(row, index) in store.playoffStandings"
            :key="row.team_id"
