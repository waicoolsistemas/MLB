...
async function fetchStats() {
  ...
  stats.value = []
  try {
    stats.value = await leagueService.getPlayoffPlayerStats(categoryId.value, selectedTeamId.value)
  } catch (err: unknown) {
    const e = err as { response?: { data?: { error?: string } } }
...
<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <div>
        <h2 class="text-lg font-semibold text-white">Metricas Jugadores (Playoffs)</h2>
    ...
    <div v-else-if="!selectedTeamId" class="text-center py-12">
      ...
      <p class="text-gray-500 text-sm max-w-sm mx-auto">
        Selecciona un equipo para ver las metricas acumuladas de playoffs.
      </p>
    </div>
    ...
    <div v-else-if="statsLoading" class="space-y-2">
      <div class="bg-navy-900 border border-navy-700 rounded-lg p-4 animate-pulse" v-for="i in 5" :key="i">
        <div class="flex items-center gap-4">
          ...
          <div class="flex gap-4"><div class="h-4 bg-navy-700 rounded w-6" v-for="j in 10" :key="j"></div></div>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="stats.length === 0" class="text-center py-12">
      ...
      <h3 class="text-white font-semibold mb-2">Sin metricas de playoffs</h3>
      <p class="text-gray-500 text-sm max-w-sm mx-auto">
        Este equipo aun no tiene estadisticas capturadas en juegos de playoffs.
      </p>
    </div>
